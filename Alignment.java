    ...
    public int getValue(){
        ...
        for(int i=1; i<=m; i++){
            for(int j=1; j<=n; j++){
                ...
                dp[i][j] = min(deltaValues[indexA][indexB]+dp[i-1][j-1], gapPenalty+dp[i-1][j], gapPenalty+dp[i][j-1]);
            }
         }
    ...
    public String[] getSolutionStrings(){
            ...
            while(!(i==0 || j==0)) {
                    ...
                    if(dp[i][j] == deltaValues[indexA][indexB]+dp[i-1][j-1]){
                        ...
                        i--;
                        j--;
                        System.out.println("Going to i-1, j-1");
                    }
            ...
            alignments[0] = sba.reverse().toString();
            alignments[1] = sbb.reverse().toString();
